...
import {
  getInmuebleById,
  hacerReserva,
  hacerReservaPresencial,
  checkReserva,
} from "../utils/inmuebles_fetch";
...
export default function ReservaButton({ id_inmueble, rol }) {
  async function handleClickCliente() {
    try {
      ...
      throw new Error("Error al validar las fechas: " + error.message);
    }
    /////
    /////
    try {
      console.log("Haciendo reserva para el inmueble con ID:", id_inmueble);
      const reserva = await checkReserva(id_inmueble,form.desde,form.hasta);
  ...
  async function handleClickEmpleado() {
    try {
      ...
      if (inmueble.ok && user) {
        const reserva = await hacerReservaPresencial(
          id_inmueble,
          form.desde,
          form.hasta,
          form.dni
  ...
  if ([undefined, null].includes(rol))
    return (
      <div>
