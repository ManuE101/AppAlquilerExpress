...
router.get("/get_Inmuebles", async (req, res) => {
    try {
        const inmuebles = await InmuebleRepository.getAllNotReserved();
...
router.post("/inmuebles_filter", async (req, res) => {
    ...
    try {
        const inmuebles = await InmuebleRepository.getAllNotReserved(); // Trae todos
        ...
        const result = inmuebles.filter(inmueble => {
          return (
            ...
            (filters.habitaciones === undefined || inmueble.habitaciones >= filters.habitaciones)
          );
        });
        res.send(result);
    } catch (error) {
